...
Forbedringsarbeidet er implementert uten produksjonssetting:

- `/admin/map` starter med oppgavevalget «Kontroller medlemsregister» eller
  «Vedlikehold grender». En trinnviser og oppgavespesifikke kontroller gjør
  neste handling synlig, mens grendeutkast beholdes ved oppgavebytte.
- Arbeidsflaten har kontrollpanel, hovedkart, kompakt kartlagmeny, handling ved
  jobben og ett resultatområde. Mobilrekkefølgen er kontroll → kart → handling
  → resultat, og kontrollene holder seg innenfor smale skjermer.
- Registerkontrollen henter adresse- og eiendomskilder samlet og er eksplisitt
  skrivebeskyttet. Matrikkeloppdatering er en egen oppfølging med forslagstekst
  og den eksisterende eksplisitte bekreftelsen på matrikkelsiden.
- En liten reducer modellerer oppgave, områdekilde, `idle`, `selecting-area`,
  `area-ready`, `fetching`, `results-ready`, `reviewing` og `error`. Gamle kall
  avbrytes, og request-ID hindrer at utdaterte svar erstatter nye resultater.
- Lagret grend er anbefalt standard. Egendefinert polygon er sekundært, og kan
  opprettes og redigeres helt med tastatur via koordinatfeltene.
- Kontrollresultatet oppsummerer «Må følges opp», «Mulige treff», «Mangler i
  register» og «Ingen avvik». Bare hentede datasett vises. Hver avviksrad viser
  kilde, forskjell, sikkerhet, neste handling og en sesjonslokal arbeidsstatus.
  Råstatuskoder brukes ikke som primærtekst.
- Kart og medlemsregister bruker samme feltrekkefølge for matrikkeldata,
  kontaktinformasjon og medlemsstatus/tilknytninger. Kartpanelet viser grend og
  e-postgrupper kompakt, bruker avbrytbar siste-svar-vinner-lagring og returnerer
  fokus etter lukking.
- Konteksttekst forklarer søkepolygon, intern grendegrense, adressepunkt,
  eiendomsteig, matrikkeldata, datakilder og hvilke data Kartverket mottar.
- Reducerovergangene har enhetstester. Et isolert Playwright-scenario passerer
  på desktop og mobil for begge oppgavene, tastaturflyt, fokus, 320/412 px
  bredde og axe WCAG 2 A/AA uten brudd.

Den menneskelige oppgavetesten kan ikke utføres av kodeagenten. Et kort,
personvernvennlig testopplegg og resultattabell ligger i
`docs/map-register-control-usability-test.md`; gjennomført test med en person
som ikke har bygget løsningen er en eksplisitt sperre før produksjonssetting.
