...
test('production schema contains complete audit triggers without sensitive values', async () => {
  ...
  assert.match(schema, /CREATE TABLE IF NOT EXISTS member_annual_fees/);
  assert.match(schema, /fee_year INTEGER NOT NULL CHECK \(fee_year BETWEEN 1900 AND 9999\)/);
  assert.match(schema, /PRIMARY KEY \(member_id, fee_year\)/);
  assert.match(schema, /member_annual_fees_audit_trigger/);
