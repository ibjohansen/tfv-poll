...
test('member mutations validate input and never permit editing property identity or attribution', async () => {
  ...
  const fee = await api.setAdminMemberAnnualFee('7', { year: 2026, paid: true });
  assert.ok(queries[1].values.includes(2026));
  assert.ok(queries[1].values.includes(true));
  assert.ok(queries[1].values.includes('admin@example.test'));
  assert.deepEqual(fee, { id: '7' });
  ...
  assert.ok(queries[2].values.includes('13')); assert.equal(created.hamlet_name, 'Slåtta Øst');
  denied = true;
