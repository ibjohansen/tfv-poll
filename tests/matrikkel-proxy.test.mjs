...
test('only the exact machine-authenticated matrikkel function bypasses the browser login proxy', async () => {
  ...
  for (const path of ['/.netlify/functions/matrikkel-sync-background', '/.netlify/functions/matrikkel-sync-background/',
    '/.netlify/functions/matrikkel-sync-background?test=1']) assert.equal(matches(path), false, path);
  for (const path of ['/admin', '/admin/members/matrikkel', '/api/admin/matrikkel/runs',
    ...
    '/.netlify/functions/other', '/.netlify/functions/survey-email-background',
