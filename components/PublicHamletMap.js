...
export default function PublicHamletMap({hamlets}) {
  ...
  if (!hamlets.length) return <section className="public-hamlet-section" aria-labelledby="public-map-title">
    <div className="public-hamlet-heading"><p className="eyebrow">Turufjell</p><h2 id="public-map-title">Grender og
      eiendommer</h2><p>Kartet er midlertidig utilgjengelig.</p></div>
  </section>;

  return <section className="public-hamlet-section" aria-labelledby="public-map-title">
    <div className="public-hamlet-heading"><p className="eyebrow">Utforsk området</p><h2 id="public-map-title">Grender
      og eiendommer</h2>
      <p>Velg en grend for å se området - klikk på vis eiendommer for å se detaljer om tomtene. Eiendomsvisningen viser
        bare H-nummer, gårds- og bruksnummer og adresse – aldri navn eller kontaktopplysninger.</p></div>
    ...
    <PublicHamletMapView hamlets={hamlets} activeHamlet={activeHamlet} properties={showProperties ? properties : []}
                         ...
                         onSelectProperty={setSelectedProperty} onError={setError}/>
    <div className="public-hamlet-status" aria-live="polite">
      {error ? <p className="form-error">{error} {showProperties &&
          <button type="button" onClick={() => loadProperties(activeId)} title="Prøv eiendomsoppslaget på nytt">Prøv
            igjen</button>}</p>
        : showProperties && !loading && <p>{properties.length} offisielle eiendommer med adresse
        i {activeHamlet?.name}. {selectedProperty && `Valgt: ${selectedProperty.address || selectedProperty.cadastralNumber}. `}{properties.filter((property) => !property.geometry).length > 0 && 'Eiendommer uten kartgeometri vises bare i listen.'}</p>}
    </div>
    {showProperties && properties.length > 0 &&
      <div className="public-property-table" role="region" aria-label={`Eiendommer i ${activeHamlet?.name}`}
           tabIndex={0}>
        <table>
          <caption>Registrerte eiendommer i {activeHamlet?.name}</caption>
          <thead>
          <tr>
            <th scope="col">H-nummer</th>
            <th scope="col">Gårds- og bruksnummer</th>
          ...
          <tbody>{properties.map((property) => <tr key={property.id}
            ...
            <th scope="row">
              <button type="button" onClick={() => setSelectedProperty(property)}
                      title="Vis eiendommen i kartet">{property.hNumber || '–'}</button>
            </th>
            <td>{property.cadastralNumber || '–'}</td>
            <td>{property.address || 'Ikke registrert'}</td>
          </tr>)}</tbody>
        </table>
      </div>}
