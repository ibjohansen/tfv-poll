...
export default function AdminMemberDirectory({ data, surveys, search, sort, direction, incompleteContact }) {
  ...
  const sortHref = (column) => `/admin/members?${new URLSearchParams({ q: search, sort: column, dir: sort === column && direction === 'asc' ? 'desc' : 'asc', ...(incompleteContact ? { contact: 'incomplete' } : {}) })}`;
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(async ([entry]) => {
      ...
      try {
        const query = new URLSearchParams({ q: search, page: String(page + 1), sort, dir: direction, ...(incompleteContact ? { contact: 'incomplete' } : {}) });
  ...
  }, [direction, hasMore, incompleteContact, loadingMore, page, search, sort]);
  useEffect(() => {
  ...
  return <>
    <div className="admin-toolbar"><button className="admin-button" type="button" onClick={() => { setExportScope(checked.size ? 'selected' : 'all'); setExportMessage(''); setExportOpen(true); }} disabled={data.mock || !surveys.length}>Eksporter til Excel{checked.size ? ` (${checked.size} valgt)` : ''}</button><Link className={`admin-button${incompleteContact ? ' is-active' : ''}`} href={incompleteContact ? `/admin/members${search ? `?q=${encodeURIComponent(search)}` : ''}` : `/admin/members?${new URLSearchParams({ ...(search ? { q: search } : {}), contact: 'incomplete' })}`}>{incompleteContact ? 'Vis alle medlemmer' : 'Vis mangelfull kontaktinfo'}</Link><button className="primary-button" type="button" onClick={create}>Nytt medlem</button></div><form className="admin-search" action="/admin/members"><label htmlFor="member-search">Søk i medlemsregisteret</label>{incompleteContact && <input type="hidden" name="contact" value="incomplete" />}<div><input id="member-search" name="q" type="search" defaultValue={search} placeholder="H-nummer, adresse, navn eller e-post" maxLength={200} /><button className="primary-button" type="submit">Søk</button>{search && <Link href={`/admin/members${incompleteContact ? '?contact=incomplete' : ''}`}>Nullstill søket</Link>}</div></form>
    <p className="admin-count" role="status">{data.total} medlemmer{incompleteContact ? ' med mangelfull hovedkontakt eller hoved-e-post' : search ? ' funnet' : ''}{data.mock ? ' · Fiktive testdata' : ''}</p>
