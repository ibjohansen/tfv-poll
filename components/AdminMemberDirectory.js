...
const fields = [
  ['h_number', 'H-nummer'], ['cadastral_number', 'Gårds- og bruksnummer', true], ['street_address', 'Gateadresse'],
...
export default function AdminMemberDirectory({ data, surveys, search, sort, direction }) {
  ...
  const sortHref = (column) => `/admin/members?${new URLSearchParams({ q: search, sort: column, dir: sort === column && direction === 'asc' ? 'desc' : 'asc' })}`;
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(async ([entry]) => {
      ...
      try {
        const query = new URLSearchParams({ q: search, page: String(page + 1), sort, dir: direction });
  ...
  }, [direction, hasMore, loadingMore, page, search, sort]);
  useEffect(() => {
  ...
  async function save(event) {
    ...
    const response = await fetch(selected.isNew ? '/api/admin/members' : `/api/admin/members/${selected.id}`, { method: selected.isNew ? 'POST' : 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    const body = await response.json();
    setSaving(false);
    if (!response.ok || !body.ok) { setMessage(body.message || 'Kunne ikke lagre medlemmet.'); return; }
    setSelected(body.member); setForm({ ...body.member, other_contact_emails: (body.member.other_contact_emails || []).join('\n') }); setNewToken(body.member.access_token || ''); setMessage(selected.isNew ? 'Medlemmet er opprettet.' : 'Lagret.'); router.refresh();
  }
  ...
  async function exportMembers(event) {
    ...
    try {
      const response = await fetch('/api/admin/members/export', {
        ...
        body: JSON.stringify({ scope: exportScope, memberIds: [...checked], surveyId: exportSurveyId }),
      });
      if (!response.ok) {
        const body = await response.json().catch(() => ({}));
