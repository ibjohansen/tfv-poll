...
function dateLabel(value, granularity, short = false) {
  ...
  return new Intl.DateTimeFormat('nb-NO', options).format(dateValue(value));
}

function Chart({ width, height, rows, granularity }) {
  ...
  return <svg width={width} height={height} role="img" tabIndex="0"
    aria-label={`Sidevisninger over tid. ${selected ? `${dateLabel(selected.date, granularity)}: ${numberFormat.format(selected.views)} sidevisninger.` : 'Bruk piltastene for å utforske punktene.'}`}
    onFocus={() => setSelectedIndex((current) => current ?? points.length - 1)} onBlur={() => setSelectedIndex(null)} onKeyDown={moveSelection}>
    <title>Sidevisninger over tid</title>
    <g transform={`translate(${margin.left},${margin.top})`}>
      ...
      <AxisBottom top={innerHeight} scale={xScale} numTicks={width < 560 ? 4 : 7} hideAxisLine tickStroke="#cfc3b3" tickFormat={(value) => dateLabel(new Date(value).toISOString().slice(0, 10), granularity, true)} tickLabelProps={() => ({ fill: axisColor, fontSize: 11, textAnchor: 'middle', dy: 6 })} />
      ...
      {selected && <g transform={`translate(${xScale(selected.value)},0)`} pointerEvents="none">
        ...
        <g transform={`translate(${xScale(selected.value) > innerWidth - 150 ? -150 : 8},8)`}>
          ...
          <text x="9" y="19" fill="#71695e" fontSize="11">{dateLabel(selected.date, granularity)}</text>
          <text x="9" y="38" fill="#27231f" fontSize="13" fontWeight="700">{numberFormat.format(selected.views)} sidevisninger</text>
        </g>
      </g>}
    </g>
  </svg>;
...
export default function UsageTrendChart({ rows, granularity }) {
  if (!rows.some((row) => row.views > 0)) return <p className="usage-empty">Ingen sidevisninger i perioden.</p>;
  return <div className="usage-chart-frame"><ParentSize debounceTime={100}>{({ width, height }) => <Chart width={width} height={height} rows={rows} granularity={granularity} />}</ParentSize></div>;
}
