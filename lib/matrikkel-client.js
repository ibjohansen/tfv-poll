...
function validateBaseUrl(value) {
  let url;
  try { url = new URL(value); } catch { throw new MatrikkelError('API_BASE_URL er ugyldig.', 'CONFIGURATION'); }
  if (url.protocol !== 'https:' || !(url.hostname === 'matrikkel.no' || url.hostname.endsWith('.matrikkel.no'))) {
    throw new MatrikkelError('API_BASE_URL må være et HTTPS-endepunkt hos matrikkel.no.', 'CONFIGURATION');
  }
...
export class MatrikkelClient {
  constructor(env = process.env) {
    if (!env.API_BASE_URL || !env.API_USR || !env.API_PWD) throw new MatrikkelError('Matrikkel-API er ikke konfigurert.', 'CONFIGURATION');
    this.baseUrl = validateBaseUrl(env.API_BASE_URL);
    ...
    const password = env.API_PWD.replaceAll('\\$', '$');
    this.authorization = `Basic ${Buffer.from(`${env.API_USR}:${password}`).toString('base64')}`;
  }
