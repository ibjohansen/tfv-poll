...
export class MatrikkelError extends Error {
  constructor(message, code = 'MATRIKKEL_ERROR', details = null) {
    super(message);
...
export class MatrikkelClient {
  ...
  async soap(service, body, options = {}) {
    ...
    const response = await fetchWithRetry(`${this.baseUrl}/${service}`, {
      ...
      body: envelope(body), signal: options.signal,
    });
  ...
  async verifyAccess(options = {}) {
    const root = await this.soap('BrukerServiceWS', `<bru:getPaloggetBruker>${matrikkelContext('bru')}</bru:getPaloggetBruker>`, options);
    ...
    const rightsRoot = await this.soap('BrukerServiceWS', `<bru:harLesEierforholdRettighet><bru:brukerId><dom:value>${xmlEscape(userId)}</dom:value></bru:brukerId>${matrikkelContext('bru')}</bru:harLesEierforholdRettighet>`, options);
  ...
  async lookupProperty({ gnr, bnr, fnr = '0', snr = '0' }, options = {}) {
    const idRoot = await this.soap('MatrikkelenhetServiceWS', `<mat:findMatrikkelenhetIdForIdent><mat:matrikkelenhetIdent><men:kommuneIdent><kom:kommunenummer>${KOMMUNENUMMER}</kom:kommunenummer></men:kommuneIdent><men:gardsnummer>${xmlEscape(gnr)}</men:gardsnummer><men:bruksnummer>${xmlEscape(bnr)}</men:bruksnummer><men:festenummer>${xmlEscape(fnr)}</men:festenummer><men:seksjonsnummer>${xmlEscape(snr)}</men:seksjonsnummer></mat:matrikkelenhetIdent>${matrikkelContext('mat')}</mat:findMatrikkelenhetIdForIdent>`, options);
    ...
    const root = await this.soap('MatrikkelenhetServiceWS', `<mat:findObjekterForMatrikkelenhet><mat:matrikkelenhetId><dom:value>${xmlEscape(matrikkelId)}</dom:value></mat:matrikkelenhetId>${matrikkelContext('mat')}</mat:findObjekterForMatrikkelenhet>`, options);
...
function candidateDetails(items) {
  return items.map((item) => ({ address: resultAddress(item), ...propertyKey(item) }));
}

function matchesExpectedProperty(item, expectedProperty) {
  if (!expectedProperty?.gnr || !expectedProperty?.bnr) return true;
  const property = propertyKey(item);
  return property.gnr === String(expectedProperty.gnr) && property.bnr === String(expectedProperty.bnr);
}

export function chooseAddressCandidate(items, address, expectedProperty = null) {
  ...
  const exactCandidates = [...distinct.values()];
  const matchingExact = exactCandidates.filter((item) => matchesExpectedProperty(item, expectedProperty));
  if (matchingExact.length === 1) return { candidate: matchingExact[0], matchType: expectedProperty ? 'EXACT_PROPERTY' : 'EXACT', score: 1 };
  if (exactCandidates.length && expectedProperty && !matchingExact.length) {
    throw new MatrikkelError('Oppgitt gårds- og bruksnummer stemmer ikke med adressen.', 'PROPERTY_MISMATCH', { candidates: candidateDetails(exactCandidates) });
  }
  if (exactCandidates.length > 1) {
    throw new MatrikkelError('Adressen har flere forskjellige eksakte treff.', 'AMBIGUOUS_ADDRESS', { candidates: candidateDetails(exactCandidates) });
  }
  ...
  if (!matchesExpectedProperty(scored[0].item, expectedProperty)) {
    throw new MatrikkelError('Oppgitt gårds- og bruksnummer stemmer ikke med adressen.', 'PROPERTY_MISMATCH', { candidates: candidateDetails([scored[0].item]) });
  }
  return { candidate: scored[0].item, matchType: 'FUZZY', score: scored[0].score };
}

async function addressRequest(query, options = {}) {
  const url = new URL(ADDRESS_API_URL);
  Object.entries(query).forEach(([key, value]) => url.searchParams.set(key, value));
  const response = await fetchWithRetry(url, { cache: 'no-store', redirect: 'error', signal: options.signal });
...
export async function lookupAddress(address, expectedProperty = null, options = {}) {
  ...
  const exactItems = await addressRequest({ ...fixed, adressetekst: value }, options);
  try { return chooseAddressCandidate(exactItems, value, expectedProperty); } catch (error) { if (['AMBIGUOUS_ADDRESS', 'PROPERTY_MISMATCH'].includes(error.code)) throw error; }
  const fuzzyItems = await addressRequest({ ...fixed, sok: value, fuzzy: 'true' }, options);
  return chooseAddressCandidate(fuzzyItems, value, expectedProperty);
}
...
export async function loadA5Entries(options = {}) {
  const response = await fetchWithRetry(A5_URL, { cache: 'no-store', redirect: 'error', signal: options.signal });
