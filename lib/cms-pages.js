...
export async function getAdminCmsPage(id) {
  ...
  const [page] = await sql`
    SELECT id, title, slug, intro, body, body_rich_text, category, image_alt, image_caption,
...
export async function createAdminCmsPage(input) {
  ...
  const sql = getSql();
  const [page] = await sql`
    INSERT INTO cms_pages (id, title, slug, intro, body, body_rich_text, category, image_alt,
      image_caption, status, published_at, last_changed_by)
    VALUES (${id}, ${input.title.trim()}, ${input.slug}, ${cleanOptional(input.intro)},
      ${rich ? richTextToPlainText(rich) : cleanOptional(input.body)}, ${rich ? JSON.stringify(rich) : null}::jsonb, ${input.category}, ${cleanOptional(input.imageAlt)},
    ...
    RETURNING id, title, slug, intro, body, body_rich_text, category, image_alt, image_caption,
      status, created_at, updated_at, published_at
  `;
...
export async function copyAdminCmsPage(sourceId) {
  ...
  const sql = getSql();
  const [source] = await sql`SELECT title, slug, intro, body, body_rich_text, category, image_alt, image_caption FROM cms_pages WHERE id = ${sourceId} AND deleted_at IS NULL`;
  if (!source) throw new Error('Page not found');
  const files = await sql`SELECT kind, title, original_filename, storage_key, mime_type, size_bytes, sort_order FROM cms_attachments WHERE page_id = ${sourceId} AND deleted_at IS NULL ORDER BY sort_order, id`;
  const id = randomUUID().replaceAll('-', '');
  const actor = user.email.toLowerCase();
  const result = await copyContentFiles(files, `pages/${id}`, (copies) => sql.transaction([
    sql`INSERT INTO cms_pages (id, title, slug, intro, body, body_rich_text, category, image_alt, image_caption, status, last_changed_by)
      VALUES (${id}, ${`Kopi av ${source.title}`.slice(0, 120)}, ${`${source.slug.slice(0, 55).replace(/-+$/, '')}-kopi-${id}`}, ${source.intro}, ${source.body}, ${source.body_rich_text === null ? null : JSON.stringify(source.body_rich_text)}::jsonb,
        ${source.category}, ${source.image_alt}, ${source.image_caption}, 'draft', ${actor}) RETURNING *`,
    ...copies.map((file) => sql`INSERT INTO cms_attachments (id, page_id, kind, title, original_filename, storage_key, mime_type, size_bytes, sort_order, last_changed_by)
      VALUES (${file.id}, ${id}, ${file.kind}, ${file.title}, ${file.original_filename}, ${file.storage_key}, ${file.mime_type}, ${file.size_bytes}, ${file.sort_order}, ${actor})`),
  ]));
...
export async function updateAdminCmsPage(id, input) {
  ...
  const [page] = await sql`
    ...
    SET title = ${input.title.trim()}, slug = ${input.slug}, intro = ${cleanOptional(input.intro)},
      body = ${rich ? richTextToPlainText(rich) : cleanOptional(input.body)}, body_rich_text = ${rich ? JSON.stringify(rich) : null}::jsonb, category = ${input.category},
  ...
  const result = await attachFiles(sql, page);
  revalidatePublicCmsContent();
  return result;
}
