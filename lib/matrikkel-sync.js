  ...
  return sql`
    WITH candidates AS (
      ...
      SELECT ${runId}, c.member_id, 'processing', c.street_address,
        jsonb_build_object('cadastral_number', c.cadastral_number, 'section_number', c.section_number, 'title_holder', c.title_holder, 'registration_date', c.registration_date)
      FROM candidates c
...
export async function processMatrikkelRun(runId, options = {}) {
  if (!/^[a-f0-9]{32}$/.test(runId)) throw new Error('Invalid run ID');
  const sql = getSql();
  const [existing] = await sql`SELECT id, status, requested_by FROM matrikkel_sync_runs WHERE id = ${runId} AND deleted_at IS NULL`;
  if (!existing) throw new Error('Run not found');
  ...
  const [started] = await sql`
    ...
    WHERE id = ${runId} AND status IN ('pending', 'running') AND deleted_at IS NULL RETURNING id
  `;
  if (!started) {
    const [current] = await sql`SELECT id, status FROM matrikkel_sync_runs WHERE id = ${runId}`;
  ...
  try {
    ...
    for (const member of members) {
      ...
      try {
        ...
        const address = await lookupAddress(member.street_address, expectedProperty);
