...
export async function getSurveyEmailOverview(surveyId, requestedPage = 1, requestedGroupId = null, { includeOtherEmails = false, memberIds = [] } = {}) {
  ...
  const [latest] = await sql`
    SELECT c.id, c.status, c.total_count, c.missing_email_count,
      ...
      c.error_message, c.created_at, c.started_at, c.completed_at, c.group_id, c.include_other_emails, g.name AS group_name
  ...
  return {
    ...
    campaign: campaignSummary(latest), deliveries, page, pages, receipts: receiptCounts,
  };
}
