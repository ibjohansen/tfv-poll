...
async function getEmailGroupMembers(sql, groupId) {
  ...
  const members = await sql`
    SELECT m.id, m.h_number, m.primary_contact_name, m.title_holder, m.primary_contact_email, m.other_contact_emails
...
async function getSelectedMembers(sql, groupId, memberIds) {
  ...
  const individual = memberIds.length ? await sql`SELECT id, h_number, primary_contact_name, title_holder, primary_contact_email, other_contact_emails
...
export async function findSurveyRecipientProperties(surveyId, search) {
  ...
  return sql`SELECT m.id::text, m.h_number, m.street_address, m.primary_contact_name
    FROM members m WHERE m.deleted_at IS NULL AND m.membership_status = 'member'
      ...
      AND (m.h_number ILIKE ${pattern} OR m.street_address ILIKE ${pattern} OR m.primary_contact_name ILIKE ${pattern})
