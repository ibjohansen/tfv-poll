...
import { buildSurveyUrl, isPastSurveyEnd, parseTestRecipients, selectCampaignRecipients, normalizeSurveyMemberIds } from './survey-email-utils.js';
...
import { buildSurveyPreviewUrl, createSurveyPreviewToken } from './survey-preview.js';
import { getSurveyMailOverview } from './survey-mail-overview.js';

const SURVEY_ID_PATTERN = /^[a-f0-9]{32}$/i;
const GROUP_ID_PATTERN = /^[1-9][0-9]{0,15}$/;
const MAILER_RETRY_CODES = new Set(['MAILERSEND_DAILY_QUOTA', 'MAILERSEND_RATE_LIMIT']);

async function issueSurveyAccessToken(sql, { memberId, surveyId, endsOn, recipientEmail }, env = process.env) {
  ...
  const result = await sql.transaction([
    sql`UPDATE survey_access_tokens SET revoked_at = NOW()
        WHERE member_id = ${memberId} AND survey_id = ${surveyId}
          AND COALESCE(recipient_email, (SELECT lower(btrim(primary_contact_email)) FROM members WHERE id = ${memberId})) = ${recipientEmail}
          AND consumed_at IS NULL AND answered_at IS NULL AND revoked_at IS NULL`,
    sql`INSERT INTO survey_access_tokens (
          id, member_id, survey_id, recipient_email, token_hash, environment, audience, expires_at
        ) VALUES (
          ${tokenId}, ${memberId}, ${surveyId}, ${recipientEmail}, ${hashAccessSecret(secret)},
...
async function getEmailGroupMembers(sql, groupId) {
  ...
  const members = await sql`
    SELECT m.id, m.h_number, m.street_address, m.primary_contact_name, m.title_holder, m.primary_contact_email, m.other_contact_emails
...
async function getSelectedMembers(sql, groupId, memberIds) {
  const grouped = groupId ? (await getEmailGroupMembers(sql, groupId)).members : [];
  const individual = memberIds.length ? await sql`SELECT id, h_number, street_address, primary_contact_name, title_holder, primary_contact_email, other_contact_emails
    FROM members WHERE id = ANY(${memberIds}::bigint[]) AND deleted_at IS NULL AND membership_status = 'member' ORDER BY id` : [];
  return [...new Map([...grouped, ...individual].map((member) => [String(member.id), member])).values()];
}
...
function campaignSummary(row) {
  ...
  return {
    ...
    include_other_emails: Boolean(row.include_other_emails),
  };
}
...
async function getCampaign(sql, campaignId) {
  const [campaign] = await sql`
    SELECT c.id, c.survey_id, c.status, c.total_count, c.missing_email_count, c.sent_count,
      c.delivered_count, c.failed_count, c.suppressed_count, c.error_message, c.retry_at, c.created_at, c.started_at, c.completed_at,
      c.group_id, c.include_other_emails, g.name AS group_name
...
export async function findSurveyRecipientProperties(surveyId, search) {
  await requirePermission('surveys');
  validateSurveyId(surveyId);
  const query = String(search || '').trim();
  if (!query || isMockMode()) return [];
  if (query.length > 200) throw new Error('Invalid member selection');
  const sql = getSql();
  // Survey admins need to select recipients, not read the full member profile.
  const pattern = `%${query.replace(/[\\%_]/g, '\\$&')}%`;
  return sql`SELECT m.id::text, m.h_number, m.street_address, m.primary_contact_name, m.title_holder, m.primary_contact_email
    FROM members m WHERE m.deleted_at IS NULL AND m.membership_status = 'member'
      AND EXISTS (SELECT 1 FROM surveys WHERE id = ${surveyId} AND deleted_at IS NULL)
      AND (m.h_number ILIKE ${pattern} OR m.street_address ILIKE ${pattern} OR m.primary_contact_name ILIKE ${pattern} OR m.title_holder ILIKE ${pattern})
    ORDER BY m.h_number, m.id LIMIT 30`;
}

export async function getSurveyEmailOverview(surveyId, requestedPage = 1, requestedGroupId = null, { includeOtherEmails = false, memberIds = [] } = {}) {
  await requirePermission('surveys');
  validateSurveyId(surveyId);
  if (isMockMode()) return { configured: false, background_configured: false, bulk_enabled: false, mock: true, groups: [], selected_group_id: null, recipients: [], recipient_count: 0, missing_email_count: 0, campaign: null, deliveries: [], page: 1, pages: 1 };
  const sql = getSql();
  const [survey] = await sql`
    SELECT id, title, is_open, ends_on, single_response_per_property,
      (EXISTS (SELECT 1 FROM email_deliveries d WHERE d.survey_id = surveys.id AND d.email_type = 'survey_invitation')
        OR EXISTS (SELECT 1 FROM survey_responses r WHERE r.survey_id = surveys.id)) AS policy_locked,
  ...
  const [latest] = await sql`
    SELECT c.id, c.status, c.total_count, c.missing_email_count,
      ...
      c.error_message, c.retry_at, c.created_at, c.started_at, c.completed_at, c.group_id, c.include_other_emails, g.name AS group_name
  ...
  const selectedGroupId = requestedGroupId === null ? campaignGroupId : requested;
  let recipients = [];
  let missingEmailCount = 0;
  const selectedMemberIds = normalizeSurveyMemberIds(memberIds);
  if (selectedGroupId || selectedMemberIds.length) {
    const members = await getSelectedMembers(sql, selectedGroupId, selectedMemberIds);
    recipients = selectCampaignRecipients(members, includeOtherEmails).map((member) => ({
      id: String(member.id), h_number: member.h_number || null, street_address: member.street_address || null,
      name: member.primary_contact_name || null, title_holder: member.title_holder || null,
      primary_contact_email: member.primaryEmail, email: member.email, is_primary: member.email === member.primaryEmail,
    }));
    missingEmailCount = members.length - new Set(recipients.map((member) => member.id)).size;
  }
  const backgroundStatus = getSurveyEmailBackgroundStatus();
  const [propertyCounts] = await sql`
    SELECT COUNT(*) FILTER (
      WHERE sent_at IS NOT NULL OR delivered_at IS NOT NULL OR status IN ('sent', 'delivered')
    )::int AS sent_emails,
    COUNT(DISTINCT member_id) FILTER (
      WHERE sent_at IS NOT NULL OR delivered_at IS NOT NULL OR status IN ('sent', 'delivered')
    )::int AS sent
    FROM email_deliveries
    WHERE survey_id = ${surveyId} AND email_type = 'survey_invitation'
  `;
  const [receiptCounts] = await sql`SELECT COUNT(*) FILTER (WHERE status IN ('pending', 'processing'))::int AS pending,
    COUNT(*) FILTER (WHERE status = 'sent')::int AS sent,
    COUNT(*) FILTER (WHERE status = 'failed')::int AS failed,
    COUNT(*) FILTER (WHERE status = 'suppressed')::int AS suppressed
    FROM survey_response_receipts WHERE survey_id = ${surveyId}`;
  const receiptIssues = await sql`
    SELECT r.id, m.h_number, m.street_address, r.recipient_email, r.status, r.failure_reason, r.created_at
    FROM survey_response_receipts r JOIN members m ON m.id = r.member_id
    WHERE r.survey_id = ${surveyId} AND r.status IN ('failed', 'suppressed')
    ORDER BY r.created_at DESC, r.id
    LIMIT 100
  `;
  return {
    configured: isMailerSendConfigured(), background_configured: backgroundStatus === 'ready', background_status: backgroundStatus,
    bulk_enabled: isMailerSendBulkEnabled(), mock: false,
    survey: { id: survey.id, title: survey.title, is_open: survey.is_open, ends_on: survey.ends_on, can_send: survey.can_send, single_response_per_property: survey.single_response_per_property, policy_locked: survey.policy_locked },
    ...
    mail_history: await getSurveyMailOverview(sql, surveyId),
    campaign: campaignSummary(latest), deliveries, page, pages, receipts: { ...receiptCounts, issues: receiptIssues },
  };
}

export async function createSurveyEmailCampaign(surveyId, { replaceCompleted = false, appendRecipients = false, groupId = null, memberIds = [], includeOtherEmails = false, singleResponsePerProperty = true } = {}) {
  ...
  if (typeof includeOtherEmails !== 'boolean' || typeof singleResponsePerProperty !== 'boolean') throw new Error('Invalid member selection');
  const sql = getSql();
  ...
  if (existing) {
    ...
    if (!appendRecipients && (!replaceCompleted || current.status !== 'completed')) {
      return { campaign: campaignSummary(current), existing: true };
    }
  }
  const selectedGroupId = normalizeGroupId(groupId);
  const selectedMemberIds = normalizeSurveyMemberIds(memberIds);
  if (!selectedGroupId && !selectedMemberIds.length) throw new Error('Invalid member selection');
  const [survey] = await sql`
    SELECT id, title, ends_on, single_response_per_property FROM surveys
  ...
  if (existing && survey.single_response_per_property !== singleResponsePerProperty) throw new Error('Invalid member selection');
  const members = await getSelectedMembers(sql, selectedGroupId, selectedMemberIds);
  const recipients = selectCampaignRecipients(members, includeOtherEmails);
  if (!recipients.length) throw new Error('No recipients');
  const adding = Boolean(existing && appendRecipients);
  const campaignId = adding ? existing.id : randomUUID().replaceAll('-', '');
  const subject = `Invitasjon: ${survey.title}`;
  const operations = [
    sql`SELECT id FROM surveys WHERE id = ${surveyId} FOR UPDATE`,
    ...(existing && !adding ? [sql`
    ...
    ...(!adding ? [sql`
      WITH policy AS (
        UPDATE surveys SET single_response_per_property = ${singleResponsePerProperty}
        WHERE id = ${surveyId} AND deleted_at IS NULL AND is_open = TRUE
          AND ends_on >= (NOW() AT TIME ZONE 'Europe/Oslo')::date
          AND (single_response_per_property = ${singleResponsePerProperty} OR (
            NOT EXISTS (SELECT 1 FROM email_deliveries WHERE survey_id = ${surveyId} AND email_type = 'survey_invitation')
            AND NOT EXISTS (SELECT 1 FROM survey_responses WHERE survey_id = ${surveyId})))
        RETURNING id
      ) INSERT INTO email_campaigns (id, survey_id, group_id, include_other_emails, requested_by, total_count, missing_email_count)
      SELECT ${campaignId}, id, ${selectedGroupId}, ${includeOtherEmails}, ${user.email.toLowerCase()}, 0,
        ${members.length - new Set(recipients.map((member) => String(member.id))).size} FROM policy
    `] : [sql`UPDATE email_campaigns SET include_other_emails = include_other_emails OR ${includeOtherEmails}
      WHERE id = ${campaignId} AND status <> 'cancelled'`]),
    ...recipients.map((member) => sql`
      INSERT INTO email_deliveries (id, campaign_id, member_id, survey_id, recipient_email, email_type, subject, source_group_id)
      SELECT ${randomUUID().replaceAll('-', '')}, ${campaignId}, ${member.id}, ${surveyId}, ${member.email}, 'survey_invitation', ${subject},
        ${selectedMemberIds.includes(String(member.id)) ? null : selectedGroupId}::bigint
      WHERE EXISTS (SELECT 1 FROM email_campaigns c JOIN surveys s ON s.id = c.survey_id
        WHERE c.id = ${campaignId} AND c.status <> 'cancelled' AND s.deleted_at IS NULL AND s.is_open = TRUE
          AND s.ends_on >= (NOW() AT TIME ZONE 'Europe/Oslo')::date)
        AND (NOT ${adding} OR NOT EXISTS (SELECT 1 FROM email_deliveries
        WHERE survey_id = ${surveyId} AND member_id = ${member.id} AND recipient_email = ${member.email} AND email_type = 'survey_invitation'))
      ON CONFLICT DO NOTHING RETURNING id
    `),
    sql`UPDATE email_campaigns c SET total_count = (SELECT COUNT(*) FROM email_deliveries WHERE campaign_id = c.id),
      status = CASE WHEN EXISTS (SELECT 1 FROM email_deliveries WHERE campaign_id = c.id AND status = 'pending')
        THEN CASE WHEN c.status = 'running' THEN 'running' ELSE 'pending' END ELSE c.status END,
      completed_at = CASE WHEN EXISTS (SELECT 1 FROM email_deliveries WHERE campaign_id = c.id AND status = 'pending') THEN NULL ELSE c.completed_at END
      WHERE c.id = ${campaignId} AND c.status <> 'cancelled'`,
  ];
  let addedCount = 0;
  try {
    const results = await sql.transaction(operations);
    const firstDeliveryIndex = 2 + (existing && !adding ? 1 : 0);
    addedCount = results.slice(firstDeliveryIndex, -1).reduce((count, rows) => count + rows.length, 0);
  }
  catch (error) {
    if (error.code !== '23505' && error.cause?.code !== '23505') throw error;
    if (adding) throw error; // An audience change must never silently become a resume.
  ...
  if (!campaign || campaign.status === 'cancelled') throw new Error('Survey not sendable');
  return { campaign: campaignSummary(campaign), existing: adding, appended: adding, added_count: addedCount, replaced: Boolean(existing && !adding) };
}
...
export async function sendSurveyTestEmail(surveyId, recipientInput) {
  ...
  if (isMockMode()) throw new Error('Mock data cannot send email');
  const sql = getSql();
  const [survey] = await sql`SELECT id, title, ends_on, question_version FROM surveys WHERE id = ${surveyId} AND deleted_at IS NULL`;
  if (!survey) throw new Error('Survey not found');
  const providerSuppressions = await getMailerSendSuppressions({ sql, context: { emailType: 'survey_test', surveyId } });
  ...
  const previewToken = createSurveyPreviewToken({ surveyId: survey.id, questionVersion: Number(survey.question_version) }, process.env);
  const testUrl = buildSurveyPreviewUrl({ baseUrl, token: previewToken }, process.env);
  ...
  for (const email of recipients) {
    ...
    `;
    try {
      const { messageId } = await sendEmail({ to: email, subject: rendered.subject, html: rendered.html, text: rendered.text, tags: ['survey-test'], context: { emailType: 'survey_test', surveyId, deliveryId, recipient: email } }, { sql });
...
export async function sendSurveyInvitationToMember({ memberId, surveyId }, options = {}) {
  ...
  const [source] = await sql`
    SELECT m.id AS member_id, m.primary_contact_email,
      s.id AS survey_id, s.title, s.ends_on, s.single_response_per_property
  ...
  const [localSuppressed] = await sql`SELECT recipient_email FROM email_suppressions WHERE recipient_email = ${email}`;
  const providerSuppressions = await getMailerSendSuppressions({ fetchImpl, env: options.env, sql, context: { emailType: 'survey_invitation', surveyId, memberId: source.member_id } });
  ...
  const access = await issueSurveyAccessToken(sql, {
    memberId: source.member_id, surveyId, endsOn: source.ends_on, recipientEmail: email,
  }, options.env || process.env);
  const surveyUrl = buildSurveyUrl({ baseUrl, accessToken: access.secret, surveyId });
  const rendered = renderSurveyInvitationEmail({ surveyTitle: source.title, endsOn: source.ends_on, surveyUrl, baseUrl, singleResponsePerProperty: source.single_response_per_property });
  ...
  try {
    const { messageId } = await sendEmail({ to: email, subject: rendered.subject, html: rendered.html, text: rendered.text, tags: ['survey-invitation'], context: { emailType: 'survey_invitation', memberId: source.member_id, surveyId, deliveryId, recipient: email } }, { fetchImpl, env: options.env, sql });
    await sql`UPDATE email_deliveries SET status = 'sent', provider_message_id = ${messageId}, sent_at = NOW() WHERE id = ${deliveryId}`;
...
async function refreshCampaignCounts(sql, campaignId) {
  const results = await sql.transaction([sql`SELECT id FROM email_campaigns WHERE id = ${campaignId} FOR UPDATE`, sql`
    UPDATE email_campaigns c SET
      ...
      retry_at = NULL, error_message = NULL
    FROM (
      SELECT campaign_id,
...
function isMailerRetry(error) {
  return error instanceof MailerServiceError && MAILER_RETRY_CODES.has(error.code);
}

function mailerRetryAt(error) {
...
async function pauseSurveyEmailCampaign(sql, campaignId, workerToken, error) {
  const retryAt = mailerRetryAt(error);
  const [campaign] = await sql`
    UPDATE email_campaigns SET status = 'pending', retry_at = ${retryAt},
      error_message = ${error.code}, completed_at = NULL
    WHERE id = ${campaignId} AND worker_token = ${workerToken}
    RETURNING *
  `;
  return campaignSummary(campaign || await getCampaign(sql, campaignId));
}
...
export async function processSurveyEmailCampaign(campaignId, options = {}) {
  ...
  const [lease] = await sql`
    UPDATE email_campaigns SET worker_token = ${workerToken},
      ...
      status = 'running', error_message = NULL, retry_at = NULL, completed_at = NULL,
  ...
  try {
    ...
    try { providerSuppressions = await getMailerSendSuppressions({ fetchImpl, env: options.env, sql, context: { emailType: 'survey_invitation', campaignId } }); }
    catch (error) {
      if (isMailerRetry(error)) return await pauseSurveyEmailCampaign(sql, campaignId, workerToken, error);
    ...
    for (let index = 0; index < batchSize; index += 1) {
      ...
      const [delivery] = await sql`
        ...
        RETURNING id, member_id, survey_id, recipient_email, source_group_id
      `;
      if (!delivery) break;
      const [source] = await sql`
        SELECT m.primary_contact_email, m.other_contact_emails, m.deleted_at,
          s.title, s.ends_on, s.is_open, s.single_response_per_property, s.deleted_at AS survey_deleted_at,
          c.include_other_emails,
          (${delivery.source_group_id || null}::bigint IS NULL OR EXISTS (
            SELECT 1 FROM member_email_group_members link
            JOIN member_email_groups g ON g.id = link.group_id AND g.deleted_at IS NULL
            WHERE link.group_id = ${delivery.source_group_id || null}::bigint AND link.member_id = m.id
          )) AS in_audience
        FROM members m CROSS JOIN surveys s
      ...
      `;
      const currentEmail = delivery.recipient_email;
      const allowedEmails = source ? selectCampaignRecipients([{ ...source, id: delivery.member_id }], source.include_other_emails).map(({ email }) => email) : [];
      if (!source || source.deleted_at ||
        source.survey_deleted_at || !source.is_open || !source.in_audience
        || isPastSurveyEnd(source.ends_on) || !allowedEmails.includes(currentEmail)) {
      ...
      const baseUrl = getApplicationBaseUrl();
      const access = await issueSurveyAccessToken(sql, {
        memberId: delivery.member_id, surveyId: delivery.survey_id, endsOn: source.ends_on, recipientEmail: currentEmail,
      });
      const surveyUrl = buildSurveyUrl({ baseUrl, accessToken: access.secret, surveyId: delivery.survey_id });
      const propertyDeliveries = await sql`SELECT DISTINCT recipient_email FROM email_deliveries WHERE survey_id = ${delivery.survey_id} AND member_id = ${delivery.member_id}
        AND email_type = 'survey_invitation' AND status NOT IN ('failed', 'bounced', 'suppressed') ORDER BY recipient_email`;
      const rendered = renderSurveyInvitationEmail({ surveyTitle: source.title, endsOn: source.ends_on, surveyUrl, baseUrl,
        singleResponsePerProperty: source.single_response_per_property, propertyRecipients: propertyDeliveries.map((item) => item.recipient_email)
          .filter((email) => selectCampaignRecipients([{ ...source, id: delivery.member_id }], true).some((recipient) => recipient.email === email)) });
      try {
        const { messageId } = await sendEmail({ to: currentEmail, subject: rendered.subject, html: rendered.html, text: rendered.text, tags: ['survey-invitation'], context: { emailType: 'survey_invitation', memberId: delivery.member_id, surveyId: delivery.survey_id, deliveryId: delivery.id, recipient: currentEmail } }, { fetchImpl, env: options.env, sql });
        await sql`UPDATE email_deliveries SET status = 'sent', provider_message_id = ${messageId}, sent_at = NOW() WHERE id = ${delivery.id}`;
      } catch (error) {
        await sql`UPDATE survey_access_tokens SET revoked_at = NOW() WHERE id = ${access.id} AND revoked_at IS NULL`;
        if (isMailerRetry(error)) {
          await sql`UPDATE email_deliveries SET status = 'pending', failure_reason = NULL,
            processing_at = NULL, failed_at = NULL WHERE id = ${delivery.id} AND status = 'processing'`;
          retryError = error;
          break;
        }
        const status = error instanceof MailerServiceError && error.code === 'SUPPRESSED' ? 'suppressed' : 'failed';
    ...
    const refreshed = await refreshCampaignCounts(sql, campaignId);
    if (retryError) return await pauseSurveyEmailCampaign(sql, campaignId, workerToken, retryError);
    return campaignSummary(refreshed);
  } finally {
