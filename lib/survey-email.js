...
export async function getSurveyEmailOverview(surveyId, requestedPage = 1) {
  ...
  const [counts] = await sql`
    ...
    FROM members
    WHERE deleted_at IS NULL AND access_revoked_at IS NULL AND access_expires_at > NOW()
  `;
...
export async function createSurveyEmailCampaign(surveyId) {
  ...
  `;
  if (existing) return { campaign: campaignSummary(await getCampaign(sql, existing.id)), existing: true };
  const [survey] = await sql`
  ...
  return { campaign: campaignSummary(await getCampaign(sql, campaignId)), existing: false };
}
...
export async function sendSurveyInvitationToMember({ memberId, surveyId }, options = {}) {
  ...
  const [source] = await sql`
    SELECT m.id AS member_id, m.access_token, m.primary_contact_email,
    ...
    WHERE m.id = ${Number(memberId)} AND m.deleted_at IS NULL
      AND m.access_revoked_at IS NULL AND m.access_expires_at > NOW()
  ...
  const surveyUrl = buildSurveyUrl({ baseUrl, accessToken: source.access_token, surveyId });
...
export async function processSurveyEmailCampaign(campaignId, options = {}) {
  ...
  try {
    ...
    for (let index = 0; index < batchSize; index += 1) {
      ...
      if (!delivery) break;
      const [source] = await sql`
        SELECT m.access_token, m.primary_contact_email, m.deleted_at, m.access_revoked_at, m.access_expires_at,
      ...
      if (!source || source.deleted_at || source.access_revoked_at || new Date(source.access_expires_at) <= new Date() ||
      ...
      const baseUrl = getApplicationBaseUrl();
      const surveyUrl = buildSurveyUrl({ baseUrl, accessToken: source.access_token, surveyId: delivery.survey_id });
      const rendered = renderSurveyInvitationEmail({ surveyTitle: source.title, endsOn: source.ends_on, surveyUrl, baseUrl });
      try {
