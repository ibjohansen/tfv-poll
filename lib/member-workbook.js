...
import { surveyLinkParameters } from '../data/survey.js';
...
const columns = [
  ...
  ['survey_url', 'Personlig undersøkelseslenke', 85],
];
...
export async function buildMemberWorkbook({ members, survey, baseUrl }) {
  ...
  for (const member of members) {
    ...
    url.searchParams.set(surveyLinkParameters.member, member.access_token);
    url.searchParams.set(surveyLinkParameters.survey, survey.id);
    const row = sheet.addRow({
      ...Object.fromEntries(columns.map(([key]) => [key, text(member[key])])),
      h_number: sortableNumber(member.h_number),
      survey_title: survey.title,
      survey_id: survey.id,
      survey_url: url.toString(),
    });
    row.getCell('survey_url').value = { text: url.toString(), hyperlink: url.toString(), tooltip: 'Åpne personlig undersøkelseslenke' };
    row.getCell('survey_url').font = { color: { argb: 'FF0563C1' }, underline: true };
