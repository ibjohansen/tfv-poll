    ...
    WITH consumed AS (
      ...
      WHERE t.token_hash = ${hashAccessSecret(secret)} AND t.consumed_at IS NULL
        ...
        AND EXISTS (
          ...
          WHERE m.id = t.member_id AND m.deleted_at IS NULL AND s.deleted_at IS NULL
...
export async function getSurveyAccess(secret, options = {}) {
  ...
  const [row] = await sql`
    ...
    WHERE m.deleted_at IS NULL AND s.deleted_at IS NULL AND s.is_open = TRUE
  `;
...
export async function submitSurveyResponse(secret, answers, options = {}) {
  ...
  const [saved] = await sql`
    WITH valid_session AS (
      ...
      FROM valid_session v JOIN surveys s ON s.id = v.survey_id
