...
export const MEMBER_ACCESS_TTL_SECONDS = 15 * 60;
export const MEMBER_SESSION_TTL_SECONDS = 45 * 60;
export const MEMBER_SESSION_ABSOLUTE_TTL_SECONDS = 8 * 60 * 60;
...
export function normalizeAddressLookup(value) {
  const lookup = cleanText(String(value || ''), 500);
  return lookup ? lookup.toLocaleLowerCase('nb-NO') : null;
}

export function normalizeCadastralNumber(value) {
  const cleaned = cleanText(String(value || ''), 50);
  if (!cleaned) return null;
  const match = cleaned.match(/^(\d+)\s*\/\s*(\d+)$/);
  if (!match || Number(match[1]) < 1 || Number(match[2]) < 1) throw new Error('Invalid cadastral number');
  return `${Number(match[1])}/${Number(match[2])}`;
}

export function parseCadastralNumber(value) {
  const normalized = normalizeCadastralNumber(value);
  if (!normalized) return null;
  const [gnr, bnr] = normalized.split('/');
  return { gnr, bnr };
}

export function normalizeSectionNumber(value) {
  const cleaned = cleanText(String(value || ''), 20);
  if (!cleaned) return null;
  if (!/^\d+$/.test(cleaned) || Number(cleaned) < 1) throw new Error('Invalid section number');
  return String(Number(cleaned));
}

export const GENERIC_MEMBER_ACCESS_MESSAGE = 'Dersom opplysningene samsvarer med et registrert medlem, sendes en e-post til den registrerte adressen.';
...
export function normalizeMembershipRequest(input) {
  ...
  if (sectionNumber && !cadastralNumber) throw new Error('Cadastral number required for section');
  return {
    h_number: hNumber,
...
export function memberSessionCookieName(env = process.env) {
  return env.NODE_ENV === 'production' ? '__Host-tfv_member_session' : 'tfv_member_session';
}
