...
export async function updateAdminMember(id, input) {
  ...
  const [member] = await sql`
    UPDATE members SET
      ...
      admin_comment = ${values.admin_comment}, last_changed_by = ${user.email.toLowerCase()}
...
export async function createAdminMember(input) {
  ...
  const sql = getSql();
  const [member] = await sql`
    INSERT INTO members (h_number, cadastral_number, section_number, street_address, primary_contact_name, primary_contact_email, other_contact_emails, admin_comment, last_changed_by)
    VALUES (${values.h_number}, ${values.cadastral_number}, ${values.section_number}, ${values.street_address}, ${values.primary_contact_name}, ${values.primary_contact_email}, ${emails}, ${values.admin_comment}, ${user.email.toLowerCase()})
...
export async function deleteAdminMember(id) {
  ...
  if (isMockMode()) throw new Error('Mock data cannot be changed');
  const sql = getSql();
  const [member] = await sql`UPDATE members SET deleted_at = NOW(), access_revoked_at = NOW(), last_changed_by = ${user.email.toLowerCase()} WHERE id = ${id} AND deleted_at IS NULL RETURNING id`;
  if (!member) throw new Error('Member not found');
}
