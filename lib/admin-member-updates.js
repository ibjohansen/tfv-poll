...
const textFields = ['h_number', 'street_address', 'primary_contact_name', 'primary_contact_email', 'admin_comment'];
...
export async function updateAdminMember(id, input) {
  ...
  const values = Object.fromEntries(textFields.map((field) => [field, nullableText(input[field])]));
  ...
  const [member] = await sql`
    UPDATE members SET
      h_number = ${values.h_number}, street_address = ${values.street_address},
    ...
    RETURNING id, h_number, cadastral_number, street_address, title_holder, registration_date,
...
export async function createAdminMember(input) {
  ...
  const sql = getSql();
  const [member] = await sql`
    INSERT INTO members (h_number, street_address, primary_contact_name, primary_contact_email, other_contact_emails, admin_comment)
    VALUES (${values.h_number}, ${values.street_address}, ${values.primary_contact_name}, ${values.primary_contact_email}, ${emails}, ${values.admin_comment})
    RETURNING id, access_token, h_number, cadastral_number, street_address, title_holder, registration_date,
      primary_contact_name, primary_contact_email, other_contact_emails, admin_comment
  `;
  return member;
}
