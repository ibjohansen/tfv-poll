...
export async function getAdminMembers(search = '', requestedPage = 1, sort = 'h_number', direction = 'asc', incompleteContact = false) {
  await requireAdmin();
  const pageSize = 25;
  ...
  if (isMockMode()) {
    const matches = mockMembers.filter((m) => Object.values(m).join(' ').toLowerCase().includes(search.toLowerCase()) &&
      (!incompleteContact || hasIncompleteContact(m)));
  ...
  const [{ count }] = await sql`
    ...
    WHERE strpos(lower(concat_ws(' ', h_number, cadastral_number, street_address,
  ...
  const members = await sql.query(
    `SELECT id, h_number, cadastral_number, street_address, title_holder,
    ...
    LIMIT $2 OFFSET $3`,
    [search, pageSize, (page - 1) * pageSize, incompleteContact],
  );
