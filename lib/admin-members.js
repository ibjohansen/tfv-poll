...
export async function getAdminMembers(search = '', requestedPage = 1, sort = 'h_number', direction = 'asc', incompleteContact = false, hasComment = false, options = {}) {
  ...
  if (isMockMode()) {
    ...
    const pageMembers = matches.slice((page - 1) * pageSize, page * pageSize)
      .map((member) => ({ ...member, email_group_ids: (member.email_group_ids || []).map(String) }));
  ...
  const [{ count }] = await sql`
    ...
    WHERE strpos(lower(concat_ws(' ', h_number, cadastral_number, section_number, street_address,
      ...
      AND (${unassignedHamlet}::boolean = FALSE OR hamlet_id IS NULL)
  ...
  const members = await sql.query(
    ...
    LIMIT $2 OFFSET $3`,
    [search, pageSize, (page - 1) * pageSize, incompleteContact, hasComment, membershipStatus, hamletId, groupId, sharing, unassignedHamlet],
  );
