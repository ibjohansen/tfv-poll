...
export function renderSurveyInvitationEmail({ surveyTitle, endsOn, surveyUrl, baseUrl, isTest = false, singleResponsePerProperty = true, propertyRecipients = [] }) {
  ...
  const testNotice = isTest ? '<p style="margin:0 0 20px;padding:12px 14px;border-radius:8px;background:#E8D288;color:#493F39"><strong>Testmelding:</strong> Lenken åpner en forhåndsvisning. Valg og testinnsending lagres ikke.</p>' : '';
  const actionLabel = isTest ? 'Forhåndsvis undersøkelsen' : 'Åpne undersøkelsen';
  ...
  const text = `${isTest ? 'TESTMELDING – lenken åpner en forhåndsvisning. Valg og testinnsending lagres ikke.\n\n' : ''}Turufjell Vel\n\n${surveyTitle}\n\nTurufjell Vel inviterer deg til å svare på undersøkelsen. Svarfristen er ${deadline}.\n\n${actionLabel}:\n${surveyUrl}\n\n${SYSTEM_EMAIL_FOOTER}`;
  const recipients = [...new Set(propertyRecipients)];
  const policy = singleResponsePerProperty
    ? 'Det registreres kun ett svar per tomt. Dersom andre enn deg får lenken og svarer først, vil dette svaret være gjeldende. Adressen som er registrert som hoved-e-post, får kvittering med spørsmål, svar og hvilken e-postadresse som sendte det tellende svaret.'
    : 'Hver invitert e-postadresse kan sende inn én selvstendig besvarelse. Hoved-e-post får kvittering på innsendte besvarelser.';
  const browserHelp = 'Hvis du får problemer med å åpne eller sende inn undersøkelsen, kan du åpne lenken i en vanlig nettleser som Safari, Chrome, Edge eller Firefox. Dette er særlig aktuelt på mobil hvis lenken først åpnes inne i e-postappen.';
