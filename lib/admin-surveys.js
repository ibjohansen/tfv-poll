...
export async function deleteAdminSurvey(id) {
  ...
  if (isMockMode()) throw new Error('Mock data cannot be changed');
  const sql = getSql();
  const [survey] = await sql`UPDATE surveys SET deleted_at = NOW(), is_open = FALSE, updated_at = NOW() WHERE id = ${id} AND deleted_at IS NULL RETURNING id`;
  if (!survey) throw new Error('Survey not found');
}
...
export async function createAdminSurvey({ title, isOpen, endsOn, questions }) {
  ...
  const sql = getSql();
  const [survey] = await sql`
    INSERT INTO surveys (id, title, is_open, ends_on, questions) VALUES (${id}, ${title.trim()}, ${isOpen}, ${endsOn}, ${JSON.stringify(normalizedQuestions)}::jsonb)
