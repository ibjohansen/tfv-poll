...
export async function processSurveyReceipts(options = {}) {
  ...
  const suppressions = await (options.getSuppressions || getMailerSendSuppressions)({ env });
  ...
  for (let index = 0; index < (options.batchSize || 100) && Date.now() + 35000 < deadline; index++) {
    ...
    try {
      const { messageId } = await send({ to, ...rendered, tags: ['survey-receipt'], context: { emailType: 'survey_receipt', memberId: receipt.member_id, surveyId: receipt.survey_id, recipient: to } }, { env });
