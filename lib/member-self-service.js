...
import {
  ...
  normalizeContactDetails, normalizeMemberLookup, normalizeMembershipRequest, randomId,
...
export async function requestMemberAccess(identifier) {
  ...
  const matches = await sql`
    ...
    ORDER BY id LIMIT 2
  `;
  if (matches.length !== 1) return { accepted: true };
  ...
  `;
  if (recent || await isRecipientSuppressed(sql, email)) return { accepted: true };
...
async function getSessionMember(secret, sql = getSql()) {
  ...
  const [member] = await sql`
    SELECT m.id, m.h_number, m.cadastral_number, m.street_address, m.title_holder,
...
export async function getMemberSelfServiceProfile(secret) {
  ...
  const [responses, deliveries, requests, updates] = await Promise.all([
    ...
    `,
    sql`
      SELECT id, request_type, status, h_number, street_address,
...
export async function updateMemberSelfServiceProfile(secret, input) {
  ...
  const [updated] = await sql`
    ...
    SELECT id, h_number, cadastral_number, street_address, title_holder, registration_date,
  ...
  try {
    const [request] = await sql`
      INSERT INTO member_requests (
        id, request_type, status, member_id, h_number, street_address,
        requested_contact_name, requested_primary_email, requested_other_emails, verified_at
      ) VALUES (
        ${randomId()}, 'ownership_transfer', 'pending', ${member.id}, ${member.h_number}, ${member.street_address},
        ${contacts.primary_contact_name}, ${contacts.primary_contact_email}, ${contacts.other_contact_emails}, NOW()
...
export async function createMembershipRequest(input) {
  const values = normalizeMembershipRequest(input);
  const sql = getSql();
  await sql`
  ...
  const [existing] = await sql`
    SELECT id FROM members WHERE deleted_at IS NULL AND (
      ...
      (${values.street_address}::text IS NOT NULL AND lower(btrim(COALESCE(street_address, ''))) = lower(${values.street_address}))
    ) LIMIT 1
  `;
  ...
  const [request] = await sql`
    INSERT INTO member_requests (
      id, request_type, status, h_number, street_address,
      requested_contact_name, requested_primary_email, requested_other_emails,
      verification_token_hash, verification_expires_at
    ) VALUES (
      ${requestId}, 'membership', 'pending_verification', ${values.h_number}, ${values.street_address},
      ${values.primary_contact_name}, ${values.primary_contact_email}, ${values.other_contact_emails},
      ${tokenHash}, NOW() + INTERVAL '24 hours'
    ) RETURNING id
  `;
...
export async function getAdminMemberRequests() {
  ...
  const sql = getSql();
  return sql`
    SELECT r.id, r.request_type, r.status, r.member_id, r.h_number, r.street_address,
...
export async function resolveAdminMemberRequest(requestId, action) {
  ...
  if (action === 'reject') {
    const [rejected] = await sql`
      ...
      WHERE id = ${requestId} AND status = 'pending' RETURNING id, status
    `;
    if (!rejected) throw new Error('Member request not found');
    return rejected;
  }
  const [request] = await sql`SELECT * FROM member_requests WHERE id = ${requestId} AND status = 'pending'`;
  ...
  const [approved] = await sql`
    WITH inserted_member AS (
      INSERT INTO members (h_number, street_address, primary_contact_name, primary_contact_email, other_contact_emails)
      SELECT COALESCE(${request.h_number}, 'N/A'), ${request.street_address}, ${request.requested_contact_name},
        ${request.requested_primary_email}, ${request.requested_other_emails}
      WHERE NOT EXISTS (
        SELECT 1 FROM members WHERE deleted_at IS NULL AND (
          ...
          (${request.street_address}::text IS NOT NULL AND lower(btrim(COALESCE(street_address, ''))) = lower(${request.street_address}))
        )
      ) RETURNING id
    )
    ...
    FROM inserted_member WHERE member_requests.id = ${requestId} AND member_requests.status = 'pending'
