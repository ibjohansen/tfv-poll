...
async function providerResponseError(response, message) {
  ...
  if (response.status === 429) {
    ...
    return new MailerServiceError(
      ...
      code,
      429,
      { retryAt: retryAtFromResponse(response, dailyQuota), providerStatus: response.status },
    );
  }
  return new MailerServiceError(`${message} (HTTP ${response.status}).`, 'UPSTREAM', 502, { providerStatus: response.status });
}
...
export async function sendEmail({ to, toName, subject, html, text, tags = [], context = {} }, options = {}) {
  ...
  if (!response.ok || !messageId) {
    ...
    throw await providerResponseError(response, 'MailerSend avviste forespørselen');
  }
