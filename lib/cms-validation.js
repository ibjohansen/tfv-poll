...
export function validateCmsPageInput(input) {
  ...
  try { sanitizeRichText(input.bodyRichText); } catch { return false; }
  return typeof input.title === 'string' && input.title.trim().length > 0 && input.title.trim().length <= 120 &&
    ...
    (input.imageDecorative === undefined || typeof input.imageDecorative === 'boolean') &&
    (input.expectedVersion === undefined || (Number.isInteger(input.expectedVersion) && input.expectedVersion > 0)) &&
    optionalText(input.overrideReason, 1000) && cmsCategories.includes(input.category) && cmsStatuses.includes(input.status);
}
