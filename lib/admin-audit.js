...
const AUDIT_SOURCE_EXPRESSION = `CASE
  WHEN changed_by ~* '^(member|applicant):' THEN 'public'
  WHEN changed_by ~* '^[^[:space:]@]+@[^[:space:]@]+\\.[^[:space:]@]+$' THEN 'admin'
  ELSE 'system'
END`;
...
export async function getAdminAuditLog(input = {}) {
  ...
  if (isMockMode()) return { entries: [], total: 0, page: 1, pageSize: 50 };
  const pageSize = 50;
  const sql = getSql();
  const values = [];
  const conditions = [];
  const add = (value, expression) => { values.push(value); conditions.push(expression(`$${values.length}`)); };
  if (filters.source) add(filters.source, (p) => `${AUDIT_SOURCE_EXPRESSION} = ${p}`);
  ...
  const [{ count }] = await sql.query(`SELECT COUNT(*)::int AS count FROM admin_activity_log ${where}`, values);
