...
export function comparisonCsv(comparison) {
  return toCsv(['Offisiell adresse', 'H-nummer', 'Gnr/Bnr Kartverket', 'Gnr/Bnr register', 'Registerstatus', 'Eier/medlem', 'E-post', 'Telefon', 'Merknad', 'Adresse i register', 'Kilder', 'Geografisk grunnlag'],
    [...comparison.rows, ...(comparison.unlocatedRows || [])].map((row) => [row.officialAddresses.map(addressLabel).join(' | '), row.register?.hNumber,
      row.officialAddresses.map(propertyLabel).join(' | '), row.register ? propertyLabel(row.register) : null, row.scope === 'unknown' ? 'Plassering ukjent – ikke klassifisert' : row.status,
      row.register?.owners?.join(' | '), row.register?.emails?.join(' | '), row.register?.phones?.join(' | '),
      row.notes.join(' '), row.register?.address, 'Kartverket (CC BY 4.0); interne registerfelt: Turufjell vel', row.scope]));
}
