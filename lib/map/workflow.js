  ...
  switch (action.type) {
    ...
    case 'fetch-succeeded':
      ...
      return { ...state, phase: MAP_PHASES.RESULTS_READY, data: action.data, busy: '', error: '',
        notice: action.notice || '', retry: null };
    case 'fetch-failed':
      if (action.requestId !== state.requestId) return state;
      return { ...state, phase: MAP_PHASES.ERROR, busy: '', error: action.error, retry: action.retry || null };
    case 'fetch-cancelled':
      if (action.requestId !== state.requestId) return state;
      return { ...state, phase: settledPhase(state), busy: '', error: '', notice: action.notice, retry: null };
    case 'review-started':
