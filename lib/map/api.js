...
export async function handleMapRequest(request, operation, { readOnly = false } = {}) {
  try {
    ...
    if (!readOnly && !request.headers.get('content-type')?.toLowerCase().startsWith('application/json')) throw new MapError('Send søkeområdet som JSON.', 415);
    const input = readOnly ? null : await readLimitedJson(request, 32_000);
    if (!readOnly && (!input || typeof input !== 'object' || Array.isArray(input))) throw new MapError('Ugyldige søkevalg.');
