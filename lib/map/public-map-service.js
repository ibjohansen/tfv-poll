...
export function normalizePublicProperties(rows, addresses) {
  ...
  return rows.map((member) => {
    ...
    return {
      ...
      latitude: Number.isFinite(official?.latitude) ? official.latitude : null,
      longitude: Number.isFinite(official?.longitude) ? official.longitude : null,
...
export async function getPublicHamletProperties(id, { signal } = {}) {
  ...
  return {
    ...
    properties,
    locatedCount: properties.filter((property) => property.latitude !== null && property.longitude !== null).length,
