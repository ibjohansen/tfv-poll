...
import { revalidatePublicHamlets } from '../public-content-cache.js';
...
export async function saveMapHamlet(input) {
  ...
  if (isMockMode()) throw new MapError('errors.hamletMock', 409);
  const sql = getSql();
  ...
  try {
    ...
    if (!rows.length) throw new MapError('errors.hamletChanged', 409);
    const hamlet = hamletRecord(rows[0]);
    revalidatePublicHamlets();
    return hamlet;
  } catch (error) {
    if ((error.code || error.cause?.code) === '23505') throw new MapError('errors.hamletDuplicate', 409);
    throw error;
  }
}
