...
export async function searchMapData(input, { signal } = {}) {
  ...
  if (hamletId !== null && (input.datatype !== 'comparison' || !/^[1-9][0-9]{0,15}$/.test(hamletId))) {
    throw new MapError('Velg en gyldig grend for registersammenligningen.');
  }
  ...
  if (input.datatype === 'comparison') {
    ...
    return { ...search, ...result, comparison: {
      ...compareRegisterWithMapData(register, result.addresses, { polygon: search.polygon, boundaries: boundaryData.boundaries }),
