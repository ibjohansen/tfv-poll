...
export function normalizeHamletInput(input) {
  if (!input || typeof input !== 'object' || Array.isArray(input)
    || !['create', 'save', 'clear'].includes(input.action)) throw new MapError('Ugyldig grendehandling.');
  const { action } = input;
  const id = action === 'create' ? null : String(input.id);
  if (id !== null && !/^[1-9][0-9]{0,15}$/.test(id)) throw new MapError('Velg en gyldig grend.');
  const version = action === 'create' ? null : input.version;
  if (action !== 'create' && (!Number.isInteger(version) || version < 1 || version >= 2147483647)) {
    throw new MapError('Last grenden på nytt før du lagrer.');
  }
  ...
  if (typeof input.name !== 'string' || !input.name.trim() || input.name.trim().length > 100 || /[\x00-\x1f\x7f]/.test(input.name)) {
    throw new MapError('Gi grenden et navn på 1–100 tegn.');
  }
  if (input.reviewed !== undefined && typeof input.reviewed !== 'boolean') throw new MapError('Ugyldig kontrollstatus.');
