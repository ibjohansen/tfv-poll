...
export function compareRegisterWithMapData(registerProperties, officialAddresses) {
  ...
  const rows = registerProperties.map((value, index) => {
    ...
    return { id: `register:${register.id ?? index}`, register, officialAddresses: candidates, status, notes };
  });
  const links = new Map();
  ...
  for (const official of officials) if (!used.has(official.id)) rows.push({
    id: `official:${official.id}`, register: null, officialAddresses: [official], status: 'MISSING_IN_REGISTER', notes: ['Offisiell adresse i polygonet uten kobling til aktivt register.'],
  });
  return {
    rows,
    counts: Object.fromEntries(COMPARISON_STATUSES.map((status) => [status, rows.filter((row) => row.status === status).length])),
    officialCount: officials.length, registerCount: registerProperties.length,
    scope: 'Hele det aktive Turufjell-registeret sammenlignes med offisielle adresser i valgt polygon.',
  };
}
