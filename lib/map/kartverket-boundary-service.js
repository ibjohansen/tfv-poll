...
const badData = () => new MapError('errors.invalidBoundaryData', 502);
const incomplete = () => new MapError('errors.incompleteBoundaries', 409);
...
function geometryFromGml(area, budget) {
  ...
  function ring(value) {
    ...
    if (budget.vertices > 150_000) throw new MapError('errors.boundariesTooDetailed', 413);
    const coordinates = [];
...
export function parseBoundaryFeatures(xml, t = (key) => key) {
  const data = collection(xml);
  const members = array(data.member);
  if (members.length > MAX_BOUNDARIES) throw new MapError('errors.tooManyBoundaries', 413, { count: MAX_BOUNDARIES });
  ...
  return members.map((member) => {
    ...
    const properties = { id, kind: 'boundary', name: references.map(propertyLabel).join(' | ') || t('labels.missingCadastral'),
...
export async function findPropertiesInPolygon(input, { fetchImpl = fetch, signal, t = (key) => key } = {}) {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 429) throw new MapError('errors.boundaryServiceBusy', 503);
        ...
        if (deadline.aborted) throw new MapError('errors.boundaryTimeout', 504);
  ...
  async function count() {
    ...
    if (Number(matched) > MAX_BOUNDARIES) throw new MapError('errors.tooManyBoundaries', 413, { count: MAX_BOUNDARIES });
    return Number(matched);
  }
  const before = await count();
  const all = parseBoundaryFeatures(await request('results'), t);
  ...
  return { boundaries, complete: true, fetchedAt: new Date().toISOString(), source: 'Kartverket / Geonorge',
    warnings: [t('warnings.boundaryGeometry'), t('warnings.boundaryCoverage')] };
}
