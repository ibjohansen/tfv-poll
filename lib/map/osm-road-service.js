...
export async function findRoadsInPolygon(input, options = {}) {
  ...
  if (!Array.isArray(data?.elements) || data.remark) throw new MapError('Overpass returnerte et ufullstendig svar. Prøv et mindre område.', 502);
  const groups = new Map();
  let vertices = 0;
  for (const way of data.elements) {
    if (way.type !== 'way' || !Number.isSafeInteger(way.id) || !Array.isArray(way.geometry) || way.geometry.length < 2) {
      throw new MapError('Overpass returnerte ugyldig veigeometri.', 502);
    }
    vertices += way.geometry.length;
    if (vertices > 30_000) throw new MapError('For mange veisegmenter. Velg et mindre område.', 413);
    const coordinates = way.geometry.map((p) => [p.lon, p.lat]);
    if (coordinates.some((p) => !p.every(Number.isFinite) || Math.abs(p[0]) > 180 || Math.abs(p[1]) > 90)) throw new MapError('Overpass returnerte ugyldige koordinater.', 502);
  ...
  return { roads: [...groups.values()].sort((a, b) => (a.name || '').localeCompare(b.name || '', 'nb-NO', { numeric: true })),
    ...
    warnings: ['Veier og stier er supplerende OSM-data, ikke en fullstendig offisiell veifortegnelse. Lengdene gjelder bare geometri innenfor polygonet.'] };
}
