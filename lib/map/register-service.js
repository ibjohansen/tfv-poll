...
export async function getRegisterProperties({ includeContacts = false } = {}) {
  await requirePermission('members');
  let rows;
  if (isMockMode()) rows = mockMembers;
  else {
    const sql = getSql();
    rows = includeContacts ? await sql`
      ...
      FROM members WHERE deleted_at IS NULL ORDER BY id LIMIT 5001
    ` : await sql`
      SELECT id, h_number, cadastral_number, section_number, street_address,
