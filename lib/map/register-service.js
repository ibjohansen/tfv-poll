...
export async function getRegisterProperties({ includeContacts = false, hamletId = null } = {}) {
  ...
  if (hamletId !== null && !/^[1-9][0-9]{0,15}$/.test(String(hamletId))) throw new MapError('errors.invalidHamlet');
  let rows;
  ...
  if (rows.length > 5000) throw new MapError('errors.registerTooLarge', 413);
