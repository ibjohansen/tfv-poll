...
export async function sendNewsletterTest(id, recipient) {
  ...
  const sql = getSql();
  const campaign = await campaignWithCounts(sql, id);
  const suppressions = await getMailerSendSuppressions({ sql, context: { emailType: 'newsletter_test', newsletterId: id } });
  ...
  try {
    const { messageId } = await sendEmail({ ...rendered, to: email, tags: ['newsletter-test'], context: { emailType: 'newsletter_test', newsletterId: id, deliveryId } }, { sql });
...
export async function processNewsletter(id, options = {}) {
  ...
  try {
    ...
    const suppressions = await getMailerSendSuppressions({ env: options.env, sql, context: { emailType: 'newsletter', newsletterId: id } });
    ...
    for (let index = 0; index < 100 && Date.now() + 35000 < deadline; index++) {
      ...
      try {
        const { messageId } = await sendEmail({ ...rendered, to: delivery.recipient_email, tags: ['newsletter'], context: { emailType: 'newsletter', newsletterId: id, deliveryId: delivery.id } }, { env: options.env, sql });
