...
const expenseFields = `id, year, batch_id, supplier, invoice_number, description, category,
  ...
  submitted_on::text, paid_on::text, notes, receipt_note, version`;
const attachmentFields = 'id, year, expense_id, original_filename, mime_type, size_bytes, suggestion';
...
export async function getAccountingOverview(value) {
  ...
  if (isMockMode()) return { year, settings: proposeAccountingYear(year, 8), years: [], memberCount: 8, paidMemberCount: 0, expenses: [], attachments: [] };
  const sql = getSql();
  const [settings, previous, years, counts, expenses, files] = await Promise.all([
    ...
    sql`SELECT COUNT(*)::int AS members, COUNT(*) FILTER (WHERE f.paid = TRUE)::int AS paid
      FROM members m LEFT JOIN member_annual_fees f ON f.member_id = m.id AND f.fee_year = ${year}
      WHERE m.deleted_at IS NULL AND m.membership_status = 'member'`,
  ...
  return { year, settings: settings[0] ? yearResult(settings[0]) : proposeAccountingYear(year, counts[0].members, previous[0] ? yearResult(previous[0]) : null),
    years: years.map((row) => row.id), memberCount: counts[0].members, paidMemberCount: counts[0].paid,
...
export async function uploadAccountingReceipt(value, file) {
  ...
  try {
    const [saved] = await sql`INSERT INTO accounting_attachments
      (id, year, original_filename, storage_key, mime_type, size_bytes, sha256, suggestion, last_changed_by)
      VALUES (${id}, ${year}, ${validated.filename}, ${key}, ${validated.mimeType}, ${validated.size},
        ${sha256}, ${JSON.stringify(suggestion)}::jsonb, ${actor})
      RETURNING id, year, expense_id, original_filename, mime_type, size_bytes, suggestion`;
  ...
  const results = await sql.transaction([
    ...
    sql`WITH input AS (
      SELECT * FROM jsonb_to_recordset(${JSON.stringify(entries)}::jsonb) AS x(
        ...
        notes text, receipt_note text, attachment_ids text[])
    ), inserted AS (
      INSERT INTO accounting_expenses (id, year, batch_id, supplier, invoice_number, description, category,
        invoice_date, currency, amount_minor, exchange_rate_million, submitted_on, paid_on, notes, receipt_note, last_changed_by)
      SELECT id, ${year}, ${batch}, supplier, invoice_number, description, category, invoice_date, currency,
        amount_minor, exchange_rate_million, submitted_on, paid_on, notes, receipt_note, ${actor} FROM input
    ...
    sql`WITH input AS (SELECT * FROM jsonb_to_recordset(${JSON.stringify(entries)}::jsonb) AS x(id text, version integer)),
      eligible AS (SELECT e.id FROM accounting_expenses e JOIN input i ON i.id = e.id AND i.version = e.version
        WHERE e.year = ${year} AND e.paid_on IS NULL
          AND (${input.action} = 'submit' OR (e.submitted_on IS NOT NULL AND e.submitted_on <= ${date}::date)))
      UPDATE accounting_expenses SET
        submitted_on = CASE WHEN ${input.action} = 'submit' THEN COALESCE(submitted_on, ${date}::date) ELSE submitted_on END,
